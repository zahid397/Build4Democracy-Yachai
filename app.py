...
# --- 9. পেইজ ২: অ্যাডমিন প্যানেল (উন্নত) ---
    ...
    if password == ADMIN_PASS:
        ...
        if st.sidebar.button("📲 Send test alert"):
            test_msg = "🔔 [TEST] YachaiBot test alert — এটা একটি টেস্ট মেসেজ।"
            ok = send_alert(test_msg)
            if ok:
                st.sidebar.success("Test alert পাঠানো হয়েছে — তোর ফোনে চেক কর।")
            else:
                st.sidebar.error("Test alert পাঠাতে ব্যর্থ। লগ চেক কর।")
            ...
            if not pending_texts:
                ...
                if not selected_index_list.empty:
                    ...
                    if st.button("ফাইনাল ট্যাগ করুন ✅", type="primary"):
                        ...
                        # ==========================================================
                        # ============== আপনার নতুন ম্যানুয়াল অ্যালার্ট লজিক (ভার্সন ৩.২) ==============
                        if status == "মিথ্যা":
                            # check score safety
                            try:
                            ...
                            if auto_send and pkt_score >= alert_threshold:
                                ...
                                if send_alert(alert_msg):
                                    logging.info(f"রিপোর্ট #{selected_index} এর জন্য অটোমেটিক অ্যালার্ট পাঠানো হয়েছে।")
                                    st.success("অটোমেটিক অ্যালার্ট পাঠানো হয়েছে।")
                                else:
                                    st.error("অটোমেটিক অ্যালার্ট পাঠাতে ব্যর্থ — লগ চেক কর।")
    ...
    st.info("এই আইটেমটি মিথ্যা হিসেবে ট্যাগ করা হয়েছে।")
    st.write(f"AI স্কোর: **{pkt_score}%** | Alert threshold: **{alert_threshold}%** | Auto-send: **{auto_send}**")

    if st.button("📨 Send Telegram alert now (manual)", key=f"manual_send_{selected_index}"):
        alert_msg = (
            f"🚨 <b>ভুয়া তথ্য শনাক্ত! (যাচাইকৃত)</b> 🚨\n\n"
            f"<b>তথ্য:</b>\n<i>{selected_row['text']}</i>\n\n"
            f"<b>AI স্কোর:</b> {pkt_score}%\n"
        ...
        if send_alert(alert_msg):
            logging.info(f"রিপোর্ট #{selected_index} এর জন্য ম্যানুয়াল অ্যালার্ট পাঠানো হয়েছে।")
            st.success("ম্যানুয়াল অ্যালার্ট পাঠানো হয়েছে।")
            st.rerun()
        else:
            st.error("ম্যানুয়াল অ্যালার্ট পাঠাতে ব্যর্থ — লগ চেক কর।")
